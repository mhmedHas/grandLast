...
//   // دالة لتحديد نوع عربية محدد
//   void _toggleVehicleType(String type) {
//     setState(() {
//       _selectedVehicleTypes[type] = !_selectedVehicleTypes[type]!;
//     });
//   }

//   // دالة لتحديد كل أنواع العربيات
//   void _selectAllVehicleTypes() {
//     setState(() {
//       bool allSelected = _selectedVehicleTypes.values.every(
//         (selected) => selected,
//       );
//       _selectedVehicleTypes.updateAll((key, value) => !allSelected);
//     });
//   }
...
//       _selectedLocationId = null;
//       _searchOfferController.clear();
//       _selectedVehicleTypes.updateAll((key, value) => false);
//     });
//   }
...
//   //   // فلترة حسب أنواع العربيات المحددة
//   //   List<Map<String, dynamic>> filteredByVehicleType = _priceOffers;

//   //   bool anyVehicleTypeSelected = _selectedVehicleTypes.values.any(
//   //     (selected) => selected,
//   //   );

//   //   if (anyVehicleTypeSelected) {
//   //     filteredByVehicleType = _priceOffers.where((offer) {
//   //       final vehicleType = (offer['vehicleType'] ?? '').toString();
//   //       return _selectedVehicleTypes.entries.any(
//   //         (entry) => entry.value && vehicleType.contains(entry.key),
//   //       );
//   //     }).toList();
//   //   }
...
//   //   // فلترة حسب نص البحث
//   //   final searchText = _searchOfferController.text.trim().toLowerCase();
//   //   if (searchText.isEmpty) return filteredByVehicleType;

//   //   return filteredByVehicleType.where((offer) {
//   //     final loading = (offer['loadingLocation'] ?? '').toString().toLowerCase();
//   //     final unloading = (offer['unloadingLocation'] ?? '')
//   //         .toString()
//   //         .toLowerCase();
...
//     // فلترة حسب نص البحث - البحث في مكان التحميل فقط
//     final searchText = _searchOfferController.text.trim().toLowerCase();
//     if (searchText.isEmpty) return filteredByVehicleType;
...
//         // فلتر أنواع العربيات - ظاهر دائماً
//         if (_selectedCompanyId != null && _priceOffers.isNotEmpty)
//           Container(
...
//                 Row(
//                   children: [
//                     Icon(Icons.filter_list, color: Colors.orange, size: 20),
//                     SizedBox(width: 8),
//                     Text(
//                       'تصفية حسب نوع العربية:',
//                       style: TextStyle(
//                         fontSize: isMobile ? 14 : 16,
//                         fontWeight: FontWeight.bold,
...
//                     return FilterChip(
//                       label: Text(type),
//                       selected: _selectedVehicleTypes[type] ?? false,
//                       onSelected: (selected) => _toggleVehicleType(type),
//                       backgroundColor: Colors.white,
//                       selectedColor: Colors.orange,
//                       checkmarkColor: Colors.white,
//                       labelStyle: TextStyle(
//                         fontSize: isMobile ? 12 : 14,
//                         color: _selectedVehicleTypes[type] ?? false
//                             ? Colors.white
//                             : Colors.black87,
//                         fontWeight: _selectedVehicleTypes[type] ?? false
//                             ? FontWeight.bold
//                             : FontWeight.normal,
//                       ),
//                       padding: EdgeInsets.symmetric(
...
//               decoration: InputDecoration(
//                 prefixIcon: Icon(Icons.search, color: Colors.orange),
//                 hintText: 'ابحث بمكان التحميل فقط...',
//                 border: OutlineInputBorder(
//                   borderRadius: BorderRadius.circular(isMobile ? 8 : 12),
//                   borderSide: BorderSide(color: Colors.orange),
//                 ),
...
//         // إظهار عدد النتائج
//         if (_isDropdownOpen && filteredOffers.isNotEmpty)
//           Padding(
//             padding: EdgeInsets.only(top: 8),
//             child: Text(
//               'عدد النتائج: ${filteredOffers.length}',
//               style: TextStyle(
//                 fontSize: isMobile ? 12 : 14,
//                 color: Colors.grey[600],
//                 fontWeight: FontWeight.w500,
...
//   Widget _buildOfferItem(Map<String, dynamic> offer, bool isMobile) {
//     // تحديد لون الخلفية إذا كان نوع العربية مطابق للفلتر
//     Color? tileColor;
//     bool anyVehicleTypeSelected = _selectedVehicleTypes.values.any(
//       (selected) => selected,
//     );
...
//     if (anyVehicleTypeSelected) {
//       final vehicleType = (offer['vehicleType'] ?? '').toString();
//       if (_selectedVehicleTypes.entries.any(
//         (entry) => entry.value && vehicleType.contains(entry.key),
//       )) {
...
//             ),
//             Text(
//               "نوع العربية: ${offer['vehicleType']} - النولون: ${offer['nolon']}",
//               style: TextStyle(
...
//             _selectedPriceOffer != null &&
//                 _selectedPriceOffer!['offerId'] == offer['offerId'] &&
//                 _selectedPriceOffer!['loadingLocation'] ==
//                     offer['loadingLocation'] &&
//                 _selectedPriceOffer!['unloadingLocation'] ==
//                     offer['unloadingLocation']
//             ? Icon(Icons.check_circle, color: Colors.green)
//             : null,
...
class _DailyWorkPageState extends State<DailyWorkPage> {
  ...
  // متغيرات جديدة للتكرار
  bool _isMultipleDrivers = false;
  int _numberOfDrivers = 1;
  List<TextEditingController> _multipleDriverControllers = [];

  @override
  void initState() {
    super.initState();
    _loadCompanies();
    _initializeMultipleDrivers();
  }

  void _initializeMultipleDrivers() {
    _multipleDriverControllers = List.generate(
      _numberOfDrivers,
      (index) => TextEditingController(),
    );
  }

  void _updateMultipleDriversCount(int newCount) {
    setState(() {
      _numberOfDrivers = newCount;

      List<String> oldTexts = _multipleDriverControllers
          .map((controller) => controller.text)
          .toList();

      _multipleDriverControllers = List.generate(newCount, (index) {
        final controller = TextEditingController();
        if (index < oldTexts.length) {
          controller.text = oldTexts[index];
        }
        return controller;
      });
    });
  ...
  Future<void> _saveSingleDailyWork(String driverName) async {
    final dailyWork = DailyWork(
      companyId: _selectedCompanyId!,
      companyName: _selectedCompanyName!,
      date: _selectedDate,
  ...
  Future<void> _saveDailyWork() async {
    ...
    if (_isMultipleDrivers) {
      bool allDriversFilled = _multipleDriverControllers.every(
        (controller) => controller.text.trim().isNotEmpty,
      );

      if (!allDriversFilled) {
        _showError('يرجى إدخال أسماء جميع السائقين');
        return;
      }
    }
    ...
    try {
      ...
      if (_isMultipleDrivers) {
        for (var driverController in _multipleDriverControllers) {
          await _saveSingleDailyWork(driverController.text.trim());
          savedCount++;
        }
      } else {
        await _saveSingleDailyWork(_driverNameController.text.trim());
        savedCount = 1;
      }

      if (_ohdaController.text.trim().isNotEmpty) {
        await _addExpenseForOhda();
      }

      _showSuccess('تم حفظ الشغل اليومي بنجاح ($savedCount رحلات)');
      _clearForm();
    } catch (e) {
      _showError('خطأ في الحفظ: $e');
    } finally {
  ...
  void _clearForm() {
    setState(() {
      ...
      _searchOfferController.clear();
      _selectedVehicleTypes.updateAll((key, value) => false);
      _isMultipleDrivers = false;
      _numberOfDrivers = 1;
      _updateMultipleDriversCount(1);
    });
  }
  ...
  Widget _buildMultipleDriversSection(bool isMobile) {
    return Column(
      children: [
        Container(
          padding: EdgeInsets.all(isMobile ? 12 : 16),
            ...
            children: [
              Row(
                children: [
                  Icon(
                    Icons.repeat,
                    color: _isMultipleDrivers ? Colors.blue : Colors.grey,
                  ),
                  SizedBox(width: 12),
                  Expanded(
                    child: Text(
                      'تكرار الرحلة لعدة سائقين',
                      style: TextStyle(
                        fontSize: isMobile ? 14 : 16,
                        fontWeight: FontWeight.bold,
                        color: _isMultipleDrivers ? Colors.blue : Colors.grey,
                  ...
                  Switch(
                    value: _isMultipleDrivers,
                    onChanged: (value) {
                      setState(() {
                        _isMultipleDrivers = value;
                        if (value) {
                          String baseDriver = _driverNameController.text.trim();
                          if (baseDriver.isNotEmpty) {
                            _multipleDriverControllers.first.text = baseDriver;
                          }
                        }
                      });
                    },
                ...
                Row(
                  children: [
                    ...
                    Container(
                      width: 100,
                      child: TextFormField(
                        initialValue: _numberOfDrivers.toString(),
                        ...
                        onChanged: (value) {
                          ...
                          if (newCount != null &&
                              newCount > 0 &&
                              newCount <= 50) {
                            _updateMultipleDriversCount(newCount);
                          }
                        },
                      ),
                    ),
              ...
              children: [
                Row(
                  children: [
                    ...
                    SizedBox(width: 8),
                    Text(
                      'أسماء السائقين (${_multipleDriverControllers.length})',
                      style: TextStyle(
                        fontSize: isMobile ? 16 : 18,
                        fontWeight: FontWeight.bold,
                        color: Colors.blue[700],
                ...
                ListView.builder(
                  ...
                  itemCount: _multipleDriverControllers.length,
                  itemBuilder: (context, index) {
                    return Padding(
                      ...
                      child: Row(
                        children: [
                          ...
                          Expanded(
                            child: TextFormField(
                              controller: _multipleDriverControllers[index],
                              style: TextStyle(fontSize: isMobile ? 14 : 16),
                              decoration: InputDecoration(
                                prefixIcon: Icon(
                                  Icons.person,
  ...
  Widget _buildDailyWorkForm(bool isMobile) {
    return Column(
      children: [
        ...
        if (!_isMultipleDrivers) _buildDriverNameField(isMobile),

        if (!_isMultipleDrivers) SizedBox(height: isMobile ? 16 : 20),

        _buildMultipleDriversSection(isMobile),
        SizedBox(height: isMobile ? 16 : 20),
  ...
  void dispose() {
    ...
    _kartaController.dispose();
    _searchOfferController.dispose();
    for (var controller in _multipleDriverControllers) {
      controller.dispose();
    }
    super.dispose();
  }
