...
//       // الحصول على التاريخ الأصلي للرحلة لحساب اليوم التالي
//       final originalDate = trip['date'] != null
//           ? (trip['date'] as DateTime)
//           : DateTime.now();
...
//         grouped[key] = {
//           'date': date,
//           'originalDate': originalDate, // حفظ التاريخ الأصلي
//           'tr': tr,
//           'description': description,
//           'vehicleType': vehicleType, // حفظ نوع العربية
//           'nolon': nolon,
//           'nolonCount': 1,
//           'overnight': companyOvernight,
//           'overnightCount': companyOvernight > 0 ? 1 : 0,
...
//       if (tripGroup['overnightCount'] > 0) {
//         // حساب تاريخ اليوم التالي للمبيت
//         final nextDay = (tripGroup['originalDate'] as DateTime).add(
//           Duration(days: 1),
//         );
//         final nextDayFormatted = DateFormat('d/M/yyyy').format(nextDay);

//         result.add({
//           'type': 'مبيت',
//           'date': nextDayFormatted, // تاريخ اليوم التالي
//           'tr': tripGroup['tr'],
//           'description':
//               '   مبيت ${tripGroup['vehicleType']}', // مبيت + اسم العربية فقط
//           'count': tripGroup['overnightCount'],
//           'price': tripGroup['overnight'],
//           'total':
//               (tripGroup['overnightCount'] as int) *
...
//       if (tripGroup['holidayCount'] > 0) {
//         // حساب تاريخ اليوم التالي للعطلة
//         final nextDay = (tripGroup['originalDate'] as DateTime).add(
//           Duration(days: 1),
//         );
//         final nextDayFormatted = DateFormat('d/M/yyyy').format(nextDay);
...
//           'tr': tripGroup['tr'],
//           'description':
//               '   عطله ${tripGroup['vehicleType']}', // عطلة + اسم العربية فقط
//           'count': tripGroup['holidayCount'],
//           'price': tripGroup['holiday'],
//           'total':
//               (tripGroup['holidayCount'] as int) *
...
class _CompanyWorkPageState extends State<CompanyWorkPage> {
  ...
  // حذف الفاتورة
  // ================================
  Future<void> _deleteInvoice(String invoiceId, List<String> tripIds) async {
    ...
    if (mounted) {
      setState(() => _isDeletingInvoice = true);
    }
    ...
    try {
      ...
      // حذف الفاتورة من Firestore
      batch.delete(_firestore.collection('invoices').doc(invoiceId));

      // تحديث الرحلات لإزالة علامة hasInvoice
      for (var tripId in tripIds) {
        batch.update(_firestore.collection('dailyWork').doc(tripId), {
          'hasInvoice': false,
        });
      ...
      // تحديث قائمة invoicedTripIds في cache
      if (_invoicedTripIdsCache.containsKey(_selectedCompanyId!)) {
        _invoicedTripIdsCache[_selectedCompanyId!]!.removeWhere(
          (id) => tripIds.contains(id),
        );
      }
      ...
      if (mounted) {
        setState(() {
          ...
          // تحديث الرحلات المتاحة للفاتورة - إضافة الرحلات المحذوفة
          final tripsToAdd = _companyWork
              .where((trip) => tripIds.contains(trip['id']))
              .toList();
          _availableTripsForInvoice.addAll(tripsToAdd);
          _availableTripsForInvoice = _sortAndGroupTripsForInvoice(
            _availableTripsForInvoice,
          );

          // إزالة الفاتورة من قائمة الفواتير
          _companyInvoices.removeWhere((inv) => inv['id'] == invoiceId);

          // تحديث cache للفواتير
          if (_companyInvoicesCache.containsKey(_selectedCompanyId!)) {
            _companyInvoicesCache[_selectedCompanyId!]!.removeWhere(
              (inv) => inv['id'] == invoiceId,
            );
          }
        });
      ...
      _showError('خطأ في حذف الفاتورة: $e');
    } finally {
      if (mounted) {
        setState(() => _isDeletingInvoice = false);
      }
    }
  }
  ...
  pdfLib.Widget _invoiceHeader(
    ...
    String companyText = company.trim();
    List words = companyText.split(' ');

    String firstTwo = words.length >= 2
        ? words.sublist(0, 2).join(' ')
        : companyText;

    String rest = words.length > 2 ? words.sublist(2).join(' ') : '';
    return pdfLib.Column(
      children: [
        pdfLib.Row(
          mainAxisAlignment: pdfLib.MainAxisAlignment.spaceBetween,
    ...
    if (usesTRSystem) {
      return pdfLib.Table(
        ...
        columnWidths: const {
          ...
          4: pdfLib.FlexColumnWidth(1), // السعر
          3: pdfLib.FlexColumnWidth(4), // البيان
          2: pdfLib.FlexColumnWidth(.8), // عدد/طن
  ...
  String _format(num v) => v.toStringAsFixed(2);
          ...
          children: [
            TableRow(
              ...
              children: const [
                ...
                TableCellHeader('الكارتة'),
                TableCellHeader('العهدة'),
                TableCellHeader('نوع السيارة'), // تم إضافة عمود نوع السيارة
                TableCellHeader('اسم الموقع'),
                TableCellHeader('مكان التعتيق'),
                TableCellHeader('مكان التحميل'),
                TableCellHeader('التاريخ'),
            ...
            ...trips.asMap().entries.map((entry) {
              ...
              return TableRow(
                ...
                children: [
                  ...
                  TableCellBody(work['karta']),
                  TableCellBody(work['ohda']),
                  TableCellBody(work['vehicleType'] ?? '-'), // نوع السيارة
                  TableCellBody(
                    work['selectedRoute'],
                    textStyle: const TextStyle(
                      fontWeight: FontWeight.bold,
            ...
            : Row(
                ...
                children: [
                  ...
                  IconButton(
                    ...
                    onPressed: _isDeletingInvoice
                        ? null
                        : () => _deleteInvoice(
                            invoice['id'],
                            List<String>.from(invoice['tripIds'] ?? []),
                          ),
